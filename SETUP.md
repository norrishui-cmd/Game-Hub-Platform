...
> 2026-08-05 P0 规则为当前真源；下方带日期的旧更新记录只保留背景，不得覆盖本页的当前发布门槛。

站点支持 English、Deutsch、日本語、Español、中文五种语言，所有公开路由均带语言前缀。
...
src/
  ...
  styles/global.css
  pages/
    robots.txt.js                     ← robots 与 Sitemap 地址由统一站点配置生成
    index.astro                       ← 根路径语言探测页
    [locale]/
      index.astro
      genres.astro
      platforms.astro
...
**域名只有一个配置源**：`src/config/site.js`。`astro.config.mjs`、robots、品牌结构化数据和联系邮箱
都从这里读取，避免不同文件再次出现域名漂移。这不代表域名已经接到 Vercel 上——
...
打开仓库 **Actions** 标签页 → "每日抓取游戏数据" → "Run workflow" 手动触发。工作流会检查三份数据
的新鲜度、已过发行日但仍标为 upcoming 的记录、构建和 SEO 审计。绿色 ✅ 后，更新的数据会推送并触发部署；
红色状态要先查看是游戏库、趋势榜、新闻源还是质量审计失败，不要忽略后继续声称“每日更新”。
...
`src/lib/games.js` 的 `isGameIndexable()` 是唯一判定源，当前必须同时满足：

- `publishStatus` 明确为 `"published"`；未设置、`draft`、`review` 都不会索引。
- 对应语言的 `summary` 至少 120 个字符。
- 至少两个有明确标题、且说明不少于 80 个字符的 `guideSections`。
- 至少两个带标签、可访问的 HTTPS 来源。
- 有真实游戏标题；自动生成的通用事实段落不能替代编辑审核。

新脚手架默认写入 `publishStatus: "draft"` 和文件对应的 `slug`。不要手改状态绕过发布流程；应先运行
`npm run game:audit -- <slug>`，再用 `npm run game:publish -- <slug>` 发布。

FAQ、发行日和新闻子页有各自独立门槛：FAQ 至少三条真实问题且每条有充分答案；发行日页需要独立的
`releaseDetails`；新闻页仅英文且至少有三条相关新闻。主游戏页通过不代表三个子页会自动进入 Sitemap。

### 已经写好的示范：`data/wiki/star-wars-zero-company.json`

这个文件的英文部分包含真实简介、攻略板块和来源，可直接参考数据结构。复制结构不等于复制内容；
每个游戏和语言版本都必须独立研究、审核并通过发布门。
...
> 历史记录：P0 后，批量生成的基础事实段落只可作为草稿起点，不能因此进入索引。
...
一段简介 + 一个"平台与发行状态"说明。它只能帮助编辑初始化结构；因为缺少两个实质攻略板块、两个
来源和明确的 published 审核状态，运行后仍保持 draft/noindex，也不会进入 Sitemap。
...
基础事实草稿；已经深挖过的游戏（以及以后继续手写的任何游戏）内容
...
> 历史记录：这些路由仍可访问，但 P0 后不再跟随主详情页自动索引。
...
两个页面仍有独立的 `<title>`/meta description/JSON-LD，但各自使用严格的独立质量门。没有真实 FAQ
或独立发行信息时会标记 noindex、从 Sitemap 排除，并且主详情页不会展示入口。
...
Sitemap 只保留 canonical、通过审核且允许索引的 URL；具体数量由当前内容质量决定，不设 URL 配额。

主详情页只在对应子页达到独立发布门槛时展示入口。
...
构建时由 `src/pages/robots.txt.js` 生成 robots，允许抓取所有页面，**没有**禁止 `/games/draft/`。这是故意的：如果在
...
主游戏页发布门要求：英文摘要至少 120 个字符、至少两个有实质说明的攻略板块、至少两个 HTTPS
来源，并拒绝 TBD、coming soon、check back 等占位文案。FAQ 为可选字段，但一旦填写就必须有完整
问题和答案；FAQ 子页另有更严格的三问门槛。其他语言只有提供独立本地化内容并通过同样门槛后才进入索引。
...
发布商账号为 `ca-pub-9505220977121599`。只有允许索引的内容页加载 AdSense 异步脚本；草稿和 noindex
页面不加载广告脚本。`public/ads.txt` 包含：
